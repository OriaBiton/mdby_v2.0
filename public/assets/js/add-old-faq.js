addEventListener('load', () => {
  ...
  document.querySelector('#file').addEventListener('change', setName);
});

async function add(e) {
  e.preventDefault();
  const file = document.getElementById('file').files[0];
  const ref = firebase.storage().ref(`images/oldFaq/${file.name}`);
  h2.innerText = null;
  ...
  uploadTask.on('state_changed', null, e => console.error(e),
    async () => {
      ...
      const add = firebase.functions().httpsCallable('addOldFaq');
...
function setName(e) {
  ...
  document.querySelector('#desc').value = name;
}
